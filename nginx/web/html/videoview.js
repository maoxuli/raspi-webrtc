				...
				janus.attach({
						...
						onmessage: function(msg, jsep) {
							...
							if(result) {
								if(result["status"]) {
									...
									// if(status === 'starting')
									// 	$('#status').removeClass('hide').text("Waiting").show();
									// else if(status === 'started')
									// 	$('#status').removeClass('hide').text("Streaming").show();
						...
						onremotestream: function(stream) {
							...
							if($('#remotevideo').length === 0) {
								addButtons = true;
								$('#stream').append('<video class="rounded centered hide" id="remotevideo" playsinline muted="muted">');
								...
								if($('#stream .no-video-container').length === 0) {
									$('#stream').append(
										'<div class="no-video-container">' +
											...
											'<span class="no-video-text">Video is not ready yet, please waiting...</span>' +
										'</div>');
								}
							} else {
...
function startStream() {
	...
	if(!selectedStream) {
		console.log("Invalid selected video id"); 
		return;
	}
	...
	if(spinner == null) {
		...
		spinner = new Spinner({position: 'absolute', top: '50%', left: '50%'}).spin(target);
	} else {
		spinner.spin();
	}
